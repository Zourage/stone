...
**Serves goal 1** (can an LLM translate this language). This is goal 1's whole measurement and the only one with a bar. (D125)
...
**Serves neither current objective** (D125), for the same reason as the coverage test above.
...
**Serves goal 1** (D125): it measures an LLM's behaviour in use rather than in learning.
...
**Serves both goals** (D125): it keeps the checks honest, and every number either goal reports rests on them.
...
**Serves both goals** (D125), for the same reason as the regression test.
...
**Goal 2's instruments live here**: `realize.py --score` and `compose.py --score` are the measurement for whether a script can translate this language, and `realize.py --ambiguous` names where the corpus leaves it undecidable. **Goal 2 has no bar** — D50 set one for goal 1 only (D125).

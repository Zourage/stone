...
|---|---|---|
| `validate.py` | no | The gate. Checks the lexicon and corpus; run before every commit. `--add FILE` is the only way words enter the lexicon (CLAUDE.md rules 4 and 6). Also errors on a held sentence copied from train (D93), on the decided rules the corpus has broken before — the two D88 pairings and the cue rule (D94) — and on `corpus/README.md` drifting from the cue list in the code or the tags in the corpus (D101); reports fronted adverbials and gloss collisions. |
| `regression_test.py` | no | Re-runs every check in `validate.py` against the pre-correction form of all 34 corrected corpus lines (`tests/fixtures/pre_correction.jsonl`), and fails if a check that used to fire on one no longer does. It also pins the 11 corrections no check can see, so widening a check shows up as a failure to update rather than as silence (D101). |

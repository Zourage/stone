...
| `compose.py` | no | Deterministic English → stone (D107). 25.3% exact against gold, and it **declines with a reason** on another 30% rather than guessing. `--roundtrip` composes, realises the result back and reports the drift as a confidence — which is the seam a model would plug into. |

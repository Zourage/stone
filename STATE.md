...
- Target: 300 lexicon entries (D54); 133 today: 53 roots from the first subagent batch (code, experiments, discussion), D58. New roots are two syllables, coined from corpus need, checked against Korean for collisions. Corpus grows alongside to ~1000 sentences for coverage.
...
## Open
- Grammar gaps G3–G11 in decisions.md (G1, G2 closed by D59, D60; can, want/should, we, quantifiers, comparison, passives, N times, compounds, while). Decide one at a time; agents leave out sentences that need them.
...
## Corpus
- 462 sentences, 55 held, `corpus/corpus.jsonl`; conventions in `corpus/README.md`. Imperative = you + intend stance. Existence and having = the hold root; no have, no exist. Equatives = second term takes the verb ending; no copula. Recipient = location case. Or and but exist; and is juxtaposition or with. If follows its clause; condition first, general evidential on it. (D43, D46, D47, D49, D51, D56, D57, D59, D60)

## Next
Decide G3–G11 in order of how often they blocked sentences (coordination, conditionals, quantifiers first). Then the next subagent batch: `scripts/merge_batch.py BATCH_DIR` to dry-run, review, `--apply`. Re-test at ~500 sentences. Open grammar: valency (passive), plural marking without a numeral.
